...
def test_get_all_jeeps():
    assert bite021.get_all_jeeps(CARS) == ['Grand Cherokee', 'Cherokee', 'Trailhawk', 'Trackhawk']
    assert "Fairlane" not in bite021.get_all_jeeps(CARS)

def test_get_first_model_each_manufacturer():
    assert bite021.get_first_model_each_manufacturer() == ['Falcon', 'Commodore', "Maxima", "Civic", "Grand Cherokee"]
